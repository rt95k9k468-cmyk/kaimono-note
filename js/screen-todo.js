...
(function () {
  ...
  function markPass(list, nowMin) {
    for (const li of list.children) {
      ...
      if (!row) continue;
      /* 用事の行の線（.tl-rail）は、丸薬の**上下だけ**を塗ります。境目は
         行のちょうど中央（＝丸薬は行の中で常に縦の中央に置かれるので、
         50%は必ず丸薬の中）に固定し、丸薬が隠します。

         行の高さぶんへ `--pass` をそのまま連続グラデーションで伸ばすと
         （前はそうでした）、行の高さは題とメモの長さで決まり丸薬の実際の
         時間とは無関係なので、丸薬の下端より下の「まだ来ていない時間」の
         線にまで色が漏れます——CLAUDE.mdが指摘していた「丸薬の下に続く
         線まで色が付く」の正体です。50%で二値に割れば、丸薬の高さが
         どうであれ境目は常に丸薬の内側に収まります。 */
