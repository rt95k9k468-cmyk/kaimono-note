...
(function () {
  ...
  function keepFieldVisible(field) {
    if (!root) return;
    /* ここで測る scrollTop は、たいてい遅すぎます。フォーカスした瞬間、
       ブラウザが「見えるところに置く」ため自分でスクロールを一度動かす
       ことがあり（フォーカスの仕様の一部）、それは focusin より先に
       効いてしまいます。だから「戻る場所」は指を置いた時点
       （pointerdown、下の wireKeyboardScroll）で覚えておき、ここでは
       まだ無いときの保険としてだけ拾います。 */
    if (kbScrollBase == null) kbScrollBase = root.scrollTop;
    const MARGIN = 16;
    const check = () => {
      if (document.activeElement !== field) return;   // 別の場所へ移っていたら何もしない
      const appEl = document.getElementById("app");
      const bottom = (appEl ? appEl.getBoundingClientRect().bottom : window.innerHeight) - MARGIN;
      const over = field.getBoundingClientRect().bottom - bottom;
      if (over > 4) KN.glideTo(root, root.scrollTop + over);
    };
    /* キーボードは出てくるのに数百msかかり、その間 --vvh も動き続けるので
       （app.js）、一度だけでなく何度か測り直します。 */
    [60, 180, 360, 620].forEach((ms) => setTimeout(check, ms));
  }
  ...
  function wireKeyboardScroll() {
    ...
    root.addEventListener("focusin", (e) => {
      const field = e.target.closest && e.target.closest("input, textarea, select");
      if (field) keepFieldVisible(field);
    });
  ...
  function render() {
    ...
    els.body.append(node(html`
      <div class="diet">
        ${/* 紙の左右に、前日・翌日の薄い端をのぞかせます。指で払っている
              あいだだけ幅を持ち（js-carousel）、隣の日があることを
              動かす前から示します。中身は払いはじめてから詰めます
              （毎回の組み直しで前後の日を作るのは無駄なので）。 */""}
        <div class="diet-carousel js-carousel">
          <div class="diet-peek is-prev js-peek-prev" aria-hidden="true"></div>
          ...
          <div class="diet-peek is-next js-peek-next" aria-hidden="true"></div>
        </div>
  ...
  /* ---------------- 横に払って、日をめくる ----------------
     ...
     指と1:1で追わせ、払っているあいだは隣の日の端を少しだけのぞかせます
     （js-carousel の左右）。縦は下に譲ります——画面ぜんぶがスクロール
     するので、向きは最初の数ピクセルで決めて、そのまま最後まで持ちます。 */
  function wireDaySwipe(el) {
    if (!el) return;
    const carousel = el.closest(".js-carousel");
    const peekPrev = carousel && carousel.querySelector(".js-peek-prev");
    const peekNext = carousel && carousel.querySelector(".js-peek-next");
    const PEEK_MAX = 68;
    const THRESHOLD = 56;
    let id = null, x0 = 0, y0 = 0, dx = 0, axis = null, frame = 0;
    let lastT = 0, lastX = 0, vx = 0;

    /* 端の中身は、払いはじめてから一度だけ詰めます。毎回の組み直しで
       前後の日ぶんまで作るのは無駄なので。 */
    const fillPeek = (panel, dir) => {
      if (!panel || panel.dataset.filled) return;
      const day = U.shiftDay(curDay(), dir);
      if (dir > 0 && day > U.todayKey()) { panel.dataset.filled = "empty"; return; }
      panel.dataset.filled = "1";
      const c = D.dayCard(day);
      panel.innerHTML = "";
      panel.append(node(html`
        <div class="diet-peek-inner">
          <span>${U.formatDay(day)}</span>
          <b class="mono-num">${c.totals ? c.totals.kcal.toLocaleString() + "kcal" : "—"}</b>
        </div>
      `));
    };
    ...
    const paint = () => {
      ...
      if (peekPrev) {
        const w = dx > 0 ? Math.min(dx, PEEK_MAX) : 0;
        peekPrev.style.width = `${w}px`;
        peekPrev.style.opacity = String(Math.min(1, w / PEEK_MAX));
      }
      if (peekNext) {
        const w = dx < 0 ? Math.min(-dx, PEEK_MAX) : 0;
        peekNext.style.width = `${w}px`;
        peekNext.style.opacity = String(Math.min(1, w / PEEK_MAX));
      }
    };
    const reset = () => {
      const t = "transform .2s var(--ease-out)";
      const tp = "width .2s var(--ease-out), opacity .2s var(--ease-out)";
      el.style.transition = t;
      if (peekPrev) peekPrev.style.transition = tp;
      if (peekNext) peekNext.style.transition = tp;
      dx = 0;
      paint();
      setTimeout(() => {
        el.style.transition = "";
        if (peekPrev) peekPrev.style.transition = "";
        if (peekNext) peekNext.style.transition = "";
      }, 220);
    };
    ...
    el.addEventListener("pointermove", (e) => {
      ...
      if (!axis) {
        ...
        if (axis === "x") {
          try { el.setPointerCapture(id); } catch (err) { /* Safari だと投げることがあります */ }
          fillPeek(peekPrev, -1); fillPeek(peekNext, 1);
        }
      }
      if (axis !== "x") return;
      ...
      /* 今日より先には行けないので、そちら向きだけ重くします。それ以外は
         指と1:1で追わせます——これまでは56pxを超えると余った分を3割にしか
         動かしておらず、それが「反応が悪い」の正体でした。 */
    ...
    const end = (e) => {
      if (e.pointerId !== id) return;
      const moved = dx;
      const fling = Math.abs(vx) > 0.5 && Math.abs(moved) >= 12;
      id = null; axis = null;
      if (Math.abs(moved) < THRESHOLD && !fling) { reset(); return; }
      ...
      slideDay(el, moved < 0 ? -1 : 1, peekPrev, peekNext);
    };
   ...
   */
  function slideDay(el, dir, peekPrev, peekNext) {
    ...
    el.style.opacity = "0";
    [peekPrev, peekNext].forEach((p) => {
      if (!p) return;
      p.style.transition = `width ${OUT_MS}ms var(--ease), opacity ${OUT_MS}ms var(--ease)`;
      p.style.width = "0px";
      p.style.opacity = "0";
    });
    setTimeout(() => {
  ...
  function buildSlotBoxes(host, day, st, opts) {
    ...
    SLOTS.forEach((sl) => {
      ...
      ta.addEventListener("input", () => {
        ...
        if (!inSheet) keepFieldVisible(ta);
