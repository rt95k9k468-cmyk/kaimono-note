...
(function () {
  ...
  function armKeyboardFollow(field) {
    ...
    kbTarget = null;
    nudgeIntoView(field);
    /* visualViewport の resize/scroll が拾えない・遅い環境向けの保険として、
       もう一度だけ後で測ります（同じ先なら nudgeIntoView は何もしません）。 */
    setTimeout(() => { if (kbField === field) nudgeIntoView(field); }, 300);
  }
  ...
  function wireKeyboardScroll() {
    ...
    /* キーボードの出入りは可視領域（visualViewport）を動かします。
       決め打ちの間隔で何度も見にいくのではなく、実際に動いたときだけ
       測り直します——同じ先にはもう動かさないので、ここで何度呼んでも
       余計な揺れにはなりません。 */
    ...
    if (vv) {
      ...
      const onChange = () => {
        clearTimeout(t);
        t = setTimeout(() => { if (kbField) nudgeIntoView(kbField); }, 50);
      };
  ...
  function wireDaySwipe(track, viewport) {
    ...
    const paint = () => {
      frame = 0;
      track.style.transform = `translateX(${-pageW + dx}px)`;
    };
    ...
    const settle = (index) => new Promise((resolve) => {
      ...
      track.style.transform = `translateX(${-pageW * index}px)`;
      setTimeout(() => {
        ...
        /* 真ん中（＝いま見ている日）に戻るときは、px の値を残さず、
           ...
           かまいません。 */
        if (index === 1) track.style.transform = "";
        resolve();
      }, 200);
    });
    ...
    const end = async (e) => {
      ...
      if (Math.abs(moved) < COMMIT && !fling) { await settle(1); return; }
      const next = U.shiftDay(curDay(), moved < 0 ? 1 : -1);
      if (next > U.todayKey()) { await settle(1); return; }
      haptic();
