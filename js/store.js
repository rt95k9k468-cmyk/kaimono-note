...
(function () {
  ...
  const CATEGORY_HINTS = [
    ...
    ["c-food",   ["米", "こめ", "パン", "麺", "めん", "パスタ", "うどん", "そば", "ラーメン", "醤油", "しょうゆ", "味噌", "みそ", "砂糖", "塩", "しお", "油", "あぶら", "小麦粉", "缶詰", "レトルト", "カレー", "お菓子", "おかし", "スナック", "調味料", "だし", "ソース", "マヨネーズ", "ケチャップ", "シリアル"]],
  ];
  ...
  function guessCategory(name) {
    ...
    for (const [catId, words] of CATEGORY_HINTS) {
      if (words.some((w) => n.includes(w))) {
        if (state.categories.some((c) => c.id === catId)) return catId;
      }
    }
