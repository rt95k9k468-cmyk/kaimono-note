...
(function () {
  ...
  /* ---------------- 色つきの絵は、もう重ねない ----------------
     ...
     **キーワード表**（`iconsV2Keys`。下の「新しく増えた絵のキーワード」で
     KEYS に足す）が要るので——外すと「病院」「体重計」のような、あとから
     増えた言葉から絵を引けなくなります。 */
  ...
  const KEYS = [
    ...
    ["cream",   ["生くりーむ", "ほいっぷ"]],
    ["yogurt",  ["よーぐると"]],
    ["iceCream",["あいす", "あいすくりーむ", "ぱるむ", "ぶらっくもんぶらん"]],
    ["pudding", ["ぷりん"]],
    ...
    ["chicken",  ["鶏", "鶏肉", "とり肉", "とりにく", "もも肉", "むね肉", "手羽", "ちきん", "ささみ", "砂ずり", "すなずり", "砂肝"]],
    ["bacon",    ["べーこん"]],
    ...
    ["rice",     ["米", "こめ", "ごはん", "ご飯", "らいす", "無洗米", "ぱっくご飯", "もち米", "玄米",
                  "ちゃーはん", "炒飯", "焼き飯", "やきめし"]],
    ["bread",    ["ぱん", "食パン", "しょくぱん", "ぶれっど", "ろーるぱん", "くるみぱん"]],
    ["croissant",["くろわっさん", "でにっしゅ"]],
    ["noodles",  ["らーめん", "麺", "春雨", "はるさめ", "びーふん"]],
    ["pasta",    ["ぱすた", "すぱげってぃ", "まかろに"]],
    ["pot",      ["鍋", "しちゅー", "すーぷ"]],
    /* 「かれーらいす」は、「はやしらいす」と同じ形の穴埋め。無いと
       「かれー」(3) が「らいす」(3) に譲って、米の絵が出ていました。
       **1語目には置かないこと**——`LABELS` が絵の名前として使うので
       （下の `KEYS.forEach`）、絵を選ぶ紙で「かれーらいす」と出ます。
       引く順番は `FLAT` が長さで決めるので、ここでの並びは効きません。 */
    ["curry",    ["かれー", "かれーらいす", "はやしらいす"]],
    ["dumpling", ["餃子", "ぎょうざ", "しゅうまい", "焼売", "肉まん"]],
    ["bento",    ["弁当", "べんとう", "惣菜", "そうざい", "おから煮", "おからに"]],
    ...
    ["cookie",     ["くっきー", "びすけっと", "お菓子", "おかし", "菓子", "あるふぉーと", "まるぼうろ", "ちょこぱい", "だっくわーず"]],
    ...
    /* 飲みもの */
    ["water",  ["水", "みず", "みねらるうぉーたー", "天然水", "炭酸水", "ぺっとぼとる"]],
    ...
    ["toast",       ["とーすと"]],
    ["bagel",       ["べーぐる"]],
    ["sweetBun",    ["菓子ぱん", "あんぱん", "くりーむぱん", "ちょこぱん", "かれーぱん"]],
    ["melonBun",    ["めろんぱん"]],
    ["pizza",       ["ぴざ", "ぴっつぁ"]],
    ...
    ["onigiri",     ["おにぎり", "おむすび"]],
    ["cupNoodle",   ["かっぷ麺", "かっぷらーめん", "かっぷぬーどる", "かっぷやきそば", "かぷぬ"]],
    ["soba",        ["そば", "蕎麦"]],
    ["udon",        ["うどん"]],
    ["somen",       ["そうめん", "ひやむぎ", "冷や麦"]],
    ["yakisoba",    ["焼きそば", "やきそば"]],
    ...
    /* 惣菜・冷凍・乾物 */
    ["tempura",     ["天ぷら", "てんぷら", "かき揚げ", "とりてん", "とり天"]],
    ["croquette",   ["ころっけ"]],
    ["friedFood",   ["ふらい", "揚げ物", "唐揚げ", "からあげ", "から揚げ", "空揚げ", "とんかつ", "めんち", "ななちき"]],
    ["gratin",      ["ぐらたん", "どりあ"]],
    ...
    ["springRoll",  ["春巻", "はるまき"]],
    /* 「ぽてさら」が無いと、かな2文字の「さら」(皿) が当たって皿の絵に
       なります（4文字なので `SHORT_NAME` の線の内側）。 */
    ["potatoSalad", ["ぽてとさらだ", "まかろにさらだ", "ぽてさら"]],
  ...
  /* 「わっくす」は整髪料の言葉。だが「わっくすがけ」「わっくすをかける」は
     床にワックスを掛ける行為の言い方で、整髪料の話ではない——ひらがなの
     言葉なので `hasWord` のカタカナ連なり判定は効かず、素の含む/含まないで
     当たってしまう。ここだけ手で除外する（この形に「床用の絵」を割り当て
     ないのは、やること側にその絵がまだ無いから——間違った絵より無いほうが
     よい、という決めごとに合わせて何も出さない）。 */
  const AVOID = { hairWax: /わっくす(がけ|をかけ)/ };

  /* ---------------- どれが主役か ---------------- */

  /* **修飾語**——それ自体は絵を持つが、同じ文に中身の語があれば譲るもの。
     「レトルト｜カレー」はカレー、「ノンアルコール｜ビール」はビール、
     「冷凍｜ピザ」はピザ。どれも修飾語のほうが**長い**ので、長さだけで
     決めると包装や形態の絵が勝ってしまいます。

     `hasWord` の丈の物差し（`TAIL_SLACK`）では、ここは分けられません
     ——「カップヌードル｜チリトマト」（頭が正解）と「ノンアルコール｜
     ビール」（後ろが正解）は同じ形だからです。分かれ目は語の性質なので、
     **語を名指しで**持ちます。単独で書かれたとき（「レトルト」「アルコール」）
     は競争相手がいないので、これまでどおりその絵が出ます。 */
  const MODIFIER = new Set([
    "れとると", "あるこーる", "冷凍", "れいとう", "冷蔵", "業務用",
  ]);

  /* 括弧の深さ。畳んだ文字列の位置でそのまま読めます——`foldRuns` は
     NFKC を通すので全角（）も ASCII になり、カタカナ→ひらがなの置き換えは
     一字が一字なので位置がずれません。

     食事メモの「食パン8枚切り2枚(チーズとポテサラとジャム)」のような書き方で、
     **括弧の中は付け合わせの注記**であって主役ではない、と言うためのもの。
     ここを「位置が先のほうを採る」で代えるのは駄目でした——日本語の複合語は
     **後ろが主名詞**（さば水煮**缶**）なので、実測で「さば水煮缶」が水になります。
     先頭が主役なのは**列挙**のときだけで、列挙かどうかは括弧と読点が言います。 */
  const OPEN = "([{「『【";
  const CLOSE = ")]}」』】";
  function depthsOf(text) {
    const d = new Array(text.length);
    let n = 0;
    for (let i = 0; i < text.length; i++) {
      if (OPEN.includes(text[i])) n++;
      d[i] = n;
      if (CLOSE.includes(text[i])) n = Math.max(0, n - 1);
    }
    return d;
  }
  ...
  function findKey(name) {
    /* 「含まれるか」ではなく「**語として**含まれるか」を聞きます
       ...
       当たったものを**その場で返しません**。`FLAT` は長さの降順なので
       「いちばん長い当たり」が先に来ますが、長さだけでは決まらないものが
       二つあります——括弧の中か外か（付け合わせの注記か主役か）と、
    ...
    /* 順位は四段。**括弧の外**が先、**修飾語でない**ほうが先、**長い**ほうが
       先、最後は**辞書に書いた順**（`FLAT` の並び＝走査の番号）。同点の決着を
       登録順のままにしてあるのは、そこを「文中で先」に代えると複合語の主名詞
       （さば水煮**缶**）を落とすから。 */
    let best = null;
    for (let i = 0; i < FLAT.length; i++) {
      const [word, icon, short] = FLAT[i];
      if (short && f.text.length > SHORT_NAME) continue;
      if (AVOID[icon] && AVOID[icon].test(f.text)) continue;
      const at = KN.util.wordAt(f, word, isWord);
      if (at < 0) continue;
      const c = { icon, depth: depths[at] || 0, mod: MODIFIER.has(word) ? 1 : 0, len: word.length };
      /* `FLAT` は長さの降順なので、あとから来るものは決して長くありません
         ——`len` で勝つことも、同点で登録順を追い越すこともない。だから
         入れ替わるのは、括弧か修飾語で前のものが退いたときだけです。 */
      if (!best || c.depth < best.depth
          || (c.depth === best.depth && c.mod < best.mod)) best = c;
    }
